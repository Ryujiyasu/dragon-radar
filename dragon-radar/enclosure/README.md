...
## 現状: v0.7 (実機適合 + 印刷前レビュー済)

3 部品クラムシェル。**ボード/ボタン現物を実測してモデル確定**、印刷前の多エージェント
レビューで確定8件(critical含む)を修正済み。外径 **ø122.6 × 厚26.9mm**。
残: 現物合わせの [VERIFY] (バレル径・ナット径・コネクタ開口角度)=テストプリントで詰める。

実測で判明した構成 (→ [`MEASURE.md`](MEASURE.md)):
- 前面=丸ディスプレイ **ø115** (有効 ø87.6) / 背面=四角キャリア **85.5×65** / 総厚 **14**
- マウントネジ4本=上下非対称な台形 (X±41.8 / 左Y32 / 右Y39)
- コネクタ: USB-C(上辺)→**底270°** / USB-A→右0° / microSD→左180° (`carrier_angle=180`)
- ボード固定: **二段締め** (カラーでボード⇔ディスプレイ → バックカバー4本で筐体固定)
- ボタン: ø12メタル(全長24/頭1.7/頭〜ナット6) → タレット cap1.8/protrude14/bore16.5

### 印刷前レビューで修正した主な点 (v0.6→v0.7)
- 🔴 **スナップ溝がトング壁0.4mmを貫通→前面リップ分離** を `wall 2.4→3.2` 等で解消(残壁1.0mm)
- 🟠 着座シェルフをガラス背面平面へ / スカラップ肉盛りを z 拘束(前後面の突き抜け防止)
- 🟡 USB-A開口 11→13mm、ナットねじ代確保(cap 1.8)
- 先行: タレットが角度合成で180°にズレmicroSDと衝突 → +X基準に統一し天面90°へ
