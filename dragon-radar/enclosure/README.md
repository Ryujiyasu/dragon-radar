...
3 部品クラムシェル + ボード mock。嵌合・固定方式・**天面ボタン配置**を固めた段階。
コネクタ角度・ボタン寸法・マウント穴は **ボード/ボタン現物で実測 → 確定** する。

```
[前面ベゼル] 表示窓 ø91.6 + 段付きスカート + 環状スナップビード + タレット逃げ
[本体タブ]   側壁 + 段付きトング(溝) + 着座シェルフ + バックカバー用ボス
             + コネクタ開口 + 天面ボタンタレット(中ぐり+配線通路)
...
- [ ] ボタン現物を実測 → `turret_protrude`/`turret_bore`/`turret_cap_th` 確定 ← 次
- [ ] ボード到着 → `[MEASURE]` 変数(コネクタ角度/マウント穴)を実測確定
