...
#define DOT_DIAMETER_PX     14
#define DOT_INVISIBLE_X     -100
...
static void draw_concentric_rings(lv_obj_t *parent)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        lv_obj_set_style_border_color(ring, DR_COLOR_RADAR_DIM, 0);
...
static void draw_crosshair(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_line_opa(h, LV_OPA_30, 0);
    ...
    lv_obj_set_style_line_opa(v, LV_OPA_30, 0);
}
...
static void create_sweep(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_arc_color(s_sweep_arc, DR_COLOR_RADAR, LV_PART_INDICATOR);
...
static void create_collected_label(lv_obj_t *parent)
{
    ...
    lv_obj_align(s_collected_label, LV_ALIGN_TOP_RIGHT, -20, 20);
}
